...
int main(int argc, char *argv[])
{
   ...
   int pid;
   char server_to_client_cgroup[BUF_SIZE];
   char cgroup_procs_file_location[BUF_SIZE];
   FILE *cgroup_procs_file;
   ...
   if (sprintf(server_to_client_fifo, "/tmp/server_to_client_fifo_%d", pid) < SUCCESS)
   {
      perror("sprintf");
      return BAD_SPRINTF;
   }
   if (sprintf(shm_location, "/shm_%d", pid) < SUCCESS)
   {
      perror("sprintf");
      return BAD_SPRINTF;
   }
   if (sprintf(server_to_client_cgroup, "/sys/fs/cgroup/client_%d", pid) < SUCCESS)
   {
      perror("sprintf");
      return BAD_SPRINTF;
   }
   if (sprintf(cgroup_procs_file_location, "/sys/fs/cgroup/client_%d/cgroup.procs", pid) < SUCCESS)
   {
      perror("sprintf");
      return BAD_SPRINTF;
   }
   ...
   // Create cgroup
   if (mkdir(server_to_client_cgroup, 0777) < SUCCESS)
   {
      perror("mkdir");
      return BAD_MKDIR;
   }

   // Open Cgroup File to add PID
   cgroup_procs_file = fopen(cgroup_procs_file_location, "a");
   ...
   if (fprintf(cgroup_procs_file, "\n%d\n", pid) < SUCCESS)
   {
      perror("write");
      return BAD_WRITE;
   }
   if (fclose(cgroup_procs_file) != SUCCESS)
   {
      perror("fclose");
      return BAD_CLOSE;
   }
   ...
   strncpy(mc.cgroup_procs_path, cgroup_procs_file_location, sizeof(mc.cgroup_procs_path));
